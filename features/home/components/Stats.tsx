  ...
  return (
    <section className="py-16 bg-[var(--color-brand-navy)] relative z-20">
      <div className="container-max">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 lg:gap-12 divide-x divide-[hsla(210,20%,98%,0.05)]">
          {INDIA_STATS.map((stat, i) => (
            <div key={stat.label} className={`text-center px-4 ${i === 0 ? "border-l-0" : ""}`}>
              <div className="flex justify-center mb-4 opacity-80">
                {icons[stat.icon as keyof typeof icons]}
              </div>
              <div className="font-display font-bold text-3xl md:text-5xl text-white mb-2 tracking-tight">
                {stat.value}<span className="text-[var(--color-brand-saffron)]">{stat.suffix}</span>
              </div>
              <div className="text-xs font-bold uppercase tracking-[0.1em] text-[var(--color-brand-muted)]">
                {stat.label}
              </div>
            </div>
          ))}
